...
#Non aged individuals are filtered out
all.sallaa6 %>%
...
# Number of individuals by suggested spatial region (remove total age = 0 )
all.sallaa6 %>%
  filter(!(sea_age_year == 0 & juvenile_age_year == 0)) %>%
...
all.sallaa6 %>%
  ...
  summarise(n=n(), .by = c(spat.unit)) %>% arrange(n) %>% summarise(med = median(n))
```
...
all.salfec %>%
  ...
  geom_tile(color = "gray30") +
  scale_fill_viridis_d() 
ggsave("figures\fec_year.png", scale = 0.8)
# by spat aggregation
all.salfec %>%
  drop_na(sai_location) %>%
...
all.salfec %>%
  #filter(!spat.unit %in% c("Baltic-sea-AU1", "Baltic-sea-AU2", "Baltic-sea-AU3")) %>%
  #filter(sai_cou_code == "FR") %>%
  #distinct(sai_location) #%>%
  distinct(spat.unit,fi_year) %>%
  summarise(count = n(), .by = spat.unit) %>%
  summarise(s = median(count))
```
...
salmon.laa %>%
  filter(!(sea_age_year == 0 & juvenile_age_year == 0)) %>%  
  filter(age.type %in%  c("both","sea.only") ) %>%
  count(sex) %>%
  mutate(tot = sum(n),
         perc = n/tot )
...
table_laa <- 
  ...
  ungroup() %>%
  select(-colnames(su_info), -spat.unit)
