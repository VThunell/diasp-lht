...
# Finnish back calculated growth
  ...
  ggplot(aes(length_mm,sea_age_year, color = sex)) +
...
swefin.sallaa <- 
  ...
  dplyr::select(sai_cou_code, fi_year, sai_location, origin, length_mm, weight_g, AU, sea_age_year, juvenile_age_year, sex, date,plus.growth) %>%
  bind_rows(fi.sallaa %>% 
              # remove individuals without length or sea age
              drop_na(length_mm) %>% 
...
lv.sallaa.b <- lv.sallaa %>% 
  ...
  dplyr::select(sai_cou_code,fi_year,sai_location,origin,length_mm,weight_g,sea_age_year,juvenile_age_year,sex,fisa_y_4326,fisa_x_4326,date,plus.growth) %>% 
              mutate(asses.unit = 5,
                     region = "Baltic-sea")
...
ie.sallaa.b <- ie.sallaa %>%
  ...
  dplyr::select(sai_cou_code,sai_su_code,fi_year,sai_location,origin,length_mm,weight_g,sea_age_year,juvenile_age_year,sex,fisa_y_4326,fisa_x_4326,date) %>%
...
all.sallaa <- swefin.sallaa %>%
  bind_rows(fr.sallaa.b, lv.sallaa.b, ie.sallaa.b)
...
all.sallaa6 %>%
  filter(!(sea_age_year == 0 & juvenile_age_year == 0)) %>%
  filter(spat.unit == "Baltic-sea-AU5",
         fi_year > 2025
         )
