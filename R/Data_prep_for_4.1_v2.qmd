...
fra.rivers2  <- fra.rivers %>%
  ...
  select(-subregion,-region,-id,-river.abb) 
...
* The french data has two sources of sex identification (observed in the field vs genetic). I use ßGenetic sex where available and complete these data with field observations. M. Nevoux considers field observations correct only from (mid) August. From the data where both genetic and field method are available, 14% are incorrect. **Keep this in mind if using this info**. The sex in Finnish (and likely in Swedish data) is determined visually when gutting the fish and far from all are determined (many NAs).
...
all.sallaa <- swe.sallaa %>%
  ...
  filter(!river %in% c("Vättern", "Vänern"), 
         !age.sea == 0) %>% 
  drop_na(length, age.sea) %>%
  dplyr::select("country", "year", "river", "origin", "length", "AU", "age.sea", "sex") %>%
  bind_rows(fin.sallaa %>% 
              # remove individuals without length or sea age
              drop_na(length, age.sea) %>% 
              dplyr::select("country", "year", "river", "origin", "length", "age.sea", "sex")) %>%
...
#                mutate(count.year = ifelse(n.years > 14, ">15","<15")) %>%
#                add_utm_columns(ll_names = c("lon", "lat"), utm_crs = 32633),
#              aes(X*1000, Y*1000), color = "deeppink", size = 0.3, show.legend = TRUE) +
#   theme_sleek()
...
# Ind counts for letter to Data request
# all.sallaa %>%
#   drop_na(river) %>%
#   summarise(count = n(), .by = c(year, region, region.gen, asses.unit)) %>%
#   mutate(spat.agg = case_when(region == "Baltic.sea" ~ paste0(region,":AU-",asses.unit),
...
all.sallaa %>%
  ...
  geom_bar(aes(ind.count, sex, fill = spat.agg), stat="identity", position = "stack") +
  scale_fill_viridis_d() 
...
all.sallaa %>%
  ...
  mutate(spat.agg = case_when(region == "Baltic.sea" ~ paste0(region,":AU-",asses.unit),
                             region == "Swedish.westcoast" ~ region, 
                             .default = region.gen)) %>%   ggplot() +
  geom_bar(aes(ind.count, spat.agg), stat="identity", position = "stack") +
  scale_fill_viridis_d() 
...
# map
plot_map_Euro +
  geom_point(data = all.salfec %>% drop_na(lat,lon) %>% add_utm_columns(ll_names = c("lon", "lat"), utm_crs = 32633), aes(X*1000, Y*1000), size = 0.3, color = "deeppink3") 
...
all.salfec %>%
  drop_na(river) %>%
  summarise(count.ind = n(), .by = c(region.gen, region, asses.unit)) %>%
  mutate(spat.agg = case_when(region == "Baltic.sea" ~ paste0(region,":AU-",asses.unit),
                             region == "Swedish.westcoast" ~ region, 
                             .default = region.gen)) %>%   
  ggplot() +
  geom_bar(aes(count.ind, spat.agg), stat="identity", position = "stack")
