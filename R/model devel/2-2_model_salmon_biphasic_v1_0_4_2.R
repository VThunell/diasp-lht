...
biph1_0_4.code <- nimbleCode({
  ...
  for(i in 1:nind){
    ...
    l_mu[1,i] <- lb_mu[su[i]] +
      (1-step(1-(age[i]+0.001)))*((exp(par_f[su[i],1])/exp(par_f[su[i],2])) - lb_mu[su[i]])*(1 - exp(-exp(par_f[su[i],2]))) +
         step(1-(age[i]+0.001))*((exp(par_f[su[i],1])/exp(par_f[su[i],2])) - lb_mu[su[i]])*(1 - exp(-exp(par_f[su[i],2])))*doy_dec[i]
  ...
  mu_par_m[1] ~ dnorm(o_mean_m, sd = 0.5)
  mu_par_f[1] ~ dnorm(o_mean_f, sd = 0.5)
  # sig_par_m[1] ~ dlnorm(log(l_sd_m), sdlog = 0.1)  
  # sig_par_f[1] ~ dlnorm(log(l_sd_f), sdlog = 0.1)  
  sig_par_m[1] ~ dlnorm(log(0.3), sdlog = 0.1)  #
  sig_par_f[1] ~ dlnorm(log(0.3), sdlog = 0.1)  
  mu_par_m[2] ~ dnorm(mean = k_mean_m, sd = 0.5)
  mu_par_f[2] ~ dnorm(mean = k_mean_f, sd = 0.5)
  sig_par_m[2] ~ dlnorm(log(k_sd_m), sdlog = 0.1)
  sig_par_f[2] ~ dlnorm(log(k_sd_f), sdlog = 0.1)
  
  # k & linf values (g and linf from fb)
  l_sd_m <- sqrt(log(1 + 278^2 / 1378^2))
  l_sd_f <- sqrt(log(1 + 30^2 / 150^2))
  l_mean_m <- log(1378) - 0.5 * l_sd_m^2 # sd of the mean based on sd:s of LKJ
  l_mean_f <- log(150) - 0.5 * l_sd_f^2 # sd of the mean based on sd:s of LKJ
  k_sd_m <- sqrt(log(1 + 0.28^2 / 0.43^2))
  k_sd_f <- sqrt(log(1 + 0.1^2 / 0.3^2)) # Fix this
...
# MCMC Samples
t <- Sys.time()
biph1_0_4.samples <- runMCMC(biph1_0_4.mcmcc, niter = 3000, nburnin = 1500, nchains = 1, WAIC = TRUE)
