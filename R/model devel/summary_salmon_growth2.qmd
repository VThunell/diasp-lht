...
pkgs <- c("tidyverse", "tidylog", "devtools","viridis","nls.multstart", "broom", "patchwork", "coda", "boot", "tidybayes","bayesplot", "nimbleHMC", "here", "purrr","posterior")
...
```{r}
node.sub <- subset_draws(laa.samples, variable = "sig_l")
node.sub %>% mcmc_trace()
summarise_draws(node.sub, median, rhat, ess_bulk, ess_tail)

node.sub <- subset_draws(laa.samples, variable = "par_f")
node.sub %>% mcmc_trace()
summarise_draws(node.sub, median, rhat, ess_bulk, ess_tail)

node.sub <- subset_draws(laa.samples, variable = "par_m")
node.sub[,sample(1:ncol(node.sub), 36)] %>% mcmc_trace()
summarise_draws(node.sub, median, rhat, ess_bulk, ess_tail) %>%
  arrange(-rhat)

node.sub <- subset_draws(laa.samples, variable = "sig_par_m")
node.sub %>% mcmc_trace()
summarise_draws(node.sub, median, rhat, ess_bulk, ess_tail) %>%
  arrange(-rhat)

node.sub <- subset_draws(laa.samples, variable = "lb_mu")
node.sub %>% mcmc_trace()
summarise_draws(node.sub, median, rhat, ess_bulk, ess_tail) %>%
  arrange(-rhat)

node.sub <- subset_draws(as_draws(laa.samples), variable = "corY")
node.sub %>% mcmc_trace()
summarise_draws(node.sub, median, rhat, ess_bulk, ess_tail) 
...
tmpP %>%
  ...
  facet_wrap(par~spat.unit, scales = "free") + 
  theme_minimal() +
