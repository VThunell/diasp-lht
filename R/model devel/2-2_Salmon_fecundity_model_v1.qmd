...
# mean-variance over location
salfec %>%
  summarise(mean = mean(n.eggs), var  = var(n.eggs),ratio = var/mean, .by = 
              sai_location) %>%
  ggplot() +
...
```{r}
#help(NUTS)
fec1.samp <- runMCMC(fec1.hmcc, niter = 10000, nburnin = 5000, nchains = 2, samplesAsCodaMCMC = TRUE, WAIC=TRUE)
...
fec.samp$samples %>%
  ...
  filter(.variable == "a") %>%
  ggplot() +
  geom_boxplot(aes(y = .value, x = factor(su)),
               outliers = FALSE) + # age parameter
  #facet_wrap(~sai_location, scales = "free") +
  theme_light()
```

```{r}
su.name <- data.fec %>%
  select(su, spat.unit, fisa_y_4326) %>%
  distinct() 
...
samp %>%
  ...
  mutate(fecundity = a*length_mm^b, .by = su ) %>%
  ggplot() +
  geom_line(aes(y = fecundity, x = length_mm, color = spat.unit), show.legend = FALSE) +
  geom_point(data = data.fec, aes(y = n.eggs, x = length_mm, color = spat.unit), show.legend = FALSE, alpha = 0.5) +
  facet_wrap(~spat.unit, scales = "fixed") 
...
nloc <- length_mm(unique(data.fec$sai_location))

# build model
fec2.model <- nimbleModel(fec2.code,
                        constants = list(nloc = nloc,
                                         nobs = nrow(data.fec),
                                         sai_location = data.fec$sai_location.int),
                        #inits=inits(),
...
# data.fec %>%
#   bind_cols(sex2 = samp$sex) %>%
#   filter(row_number() %in% which(is.na(data.fec$sex))) %>%
#   ggplot(aes(age.tot, length_mm, color = factor(sex2))) +
#   geom_point()
...
samp %>%
  ...
  geom_point(data = data.fec, aes(y = n.eggs, x = length_mm, color = sai_location), alpha = 0.5) +
...
nloc <- length(unique(data.fec$sai_location))
...
fec3.samp <- runMCMC(fec3.hmcc, niter = 5000, nburnin = 3000, nchains = 1, samplesAsCodaMCMC = TRUE, WAIC=TRUE)
...
fec4.model <- nimbleModel(fec4.code,
                        constants = list(nloc = nloc,
                                         ...
                                         sai_location = data.fec$sai_location.int),
                        #inits=inits(),
                        data = data.fec %>% select(n.eggs, length_mm, weight_g),
                        buildDerivs = TRUE)
...
<!--   geom_point(data = data.fec, aes(age.tot, length_mm), alpha = 0.1, color = "lightblue") + -->
