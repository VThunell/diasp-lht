...
# configure..
biph5.confmcmc <- configureMCMC(biph5.c, monitors = c("sig.l", "Ustar", "lb.mu","smp","g","sig.par","smo.age","mu.par","par", "l.mu"), useConjugacy = FALSE, project = biph5.model, enableWAIC = TRUE)
...
# MCMC Samples
biph5.samp <- runMCMC(biph5.mcmcc, niter = 30000, nburnin = 20000, nchains = 2, samplesAsCodaMCMC = TRUE, WAIC = TRUE, thin = 5)
...
# MCMC Samples
biph5.con.samp <- runMCMC(biph5.con.mcmcc, niter = 20000, nburnin = 15000, nchains = 2, samplesAsCodaMCMC = TRUE, WAIC = TRUE, thin = 5)
...
# remove NA nodes (the older-than-catch-age nodes)
cm <- lapply(biph5.con.samp$samples, as.matrix)
nonNA_nodes <- cm %>% 
  map(~ colnames(.x)[colSums(is.na(.x)) == 0]) %>%
  reduce(intersect) # applies function intersect over elements of the lists returned by map(), i.e. removing the colnames from both lists.
cm_cleaned <- lapply(cm, function(x) x[, nonNA_nodes, drop = FALSE])
