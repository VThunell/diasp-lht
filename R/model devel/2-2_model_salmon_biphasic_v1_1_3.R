...
biph.code <- nimbleCode({
  ...
  l_mean_m <- log(1378) - 0.5 * (l_sd_m^2)
  l_mean_f <- log(185) - 0.5 * (l_sd_f^2)
  k_sd_m <- sqrt(log(1 + (0.28^2) / (0.43^2)))
  k_sd_f <- sqrt(log(1 + (0.28^2) / (0.43^2))) # TEST THIS Value
  k_mean_m <- log(0.43) - 0.5 * (k_sd_m^2)
  k_mean_f <- log(0.43) - 0.5 * (k_sd_f^2) 
  ...
  par_mu_f[1] ~ dnorm(l_mean_f, sd = 0.2)
  par_mu_f[2] ~ dnorm(k_mean_f, sd = 0.2)
  par_sig_f[1] ~ dlnorm(log(l_sd_f), sdlog = 0.2)
  par_sig_f[2] ~ dlnorm(log(k_sd_f), sdlog = 0.2)
  l_mean_m_bs[1] ~ dnorm(l_mean_m, sd = .15)
  l_mean_m_bs[2] ~ dnorm(l_mean_m, sd = .15)
  k_mean_m_bs[1] ~ dnorm(k_mean_m, sd = .15)
  k_mean_m_bs[2] ~ dnorm(k_mean_m, sd = .15)
  l_sd_m_bs[1] ~ dlnorm(log(l_sd_m), sdlog = 0.2)
  l_sd_m_bs[2] ~ dlnorm(log(l_sd_m), sdlog = 0.2)
  k_sd_m_bs[1] ~ dlnorm(log(k_sd_m), sdlog = 0.2)
  k_sd_m_bs[2] ~ dlnorm(log(k_sd_m), sdlog = 0.2)
  
  # Baltic and Atlantic basins
  for(i in 1:nsu){
    par_mu_m[i] ~ dnorm(l_mean_m_bs[subs[i]], sd = 0.2)
    par_mu_m[(nsu+i)] ~ dnorm(k_mean_m_bs[subs[i]], sd = 0.2)
    par_sig_m[i] ~ dlnorm(log(l_sd_m_bs[subs[i]]), sdlog = 0.1)
    par_sig_m[(nsu+i)] ~ dlnorm(log(k_sd_m_bs[subs[i]]), sdlog = 0.1)
  }
...
inits <- function(){
  list(sig_l = rexp(1,1/150),
       ...
       par_sig_m = c(rlnorm(consts$nsu,log(0.2),sdlog = 0.1),
                     rlnorm(consts$nsu,log(0.6),sdlog = 0.1)),
       par_sig_f = c(rlnorm(1,log(0.2),sdlog = 0.1),
                     rlnorm(1,log(0.6),sdlog = 0.1)),
       par_mu_m = c(rnorm(consts$nsu,7,0.2),
                    rnorm(consts$nsu,-1,0.4)),
       par_mu_f = c(rnorm(1,2,0.2),
                    rnorm(1,-1,0.6)),
...
# simulated_groups <- biph.model$par_f # replace with your node name
...
# hist(exp(simulated_groups[,1]), breaks = 50)
...
biph.samples <- runMCMC(biph.mcmcc, niter = 5000, nburnin = 2000, thin = 2, nchains = 1, samplesAsCodaMCMC = TRUE, WAIC = TRUE)
Sys.time() - t
