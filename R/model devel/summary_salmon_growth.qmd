...
```{r}
laa.samplesWAIC <- readRDS("/Users/vitl0001/gitProjects/diasp-lht/data/biph1_1_2_samples_2026-04-14.RData")
...
laa.samplesWAIC$WAIC # 667375.3 14/4
...
node.sub <- subset_draws(as_draws(laa.samples), variable = "sig_par_f")
node.sub %>% mcmc_trace()
node.sub %>% summarise_draws(c("mean","median","sd","rhat","ess_bulk","ess_tail")) %>% mutate(mean_n = exp(mean))

node.sub <- subset_draws(as_draws(laa.samples), variable = "sig_par_m")
node.sub %>% mcmc_trace()
node.sub %>% summarise_draws(c("mean","median","sd","rhat","ess_bulk","ess_tail")) %>% mutate(mean_n = exp(mean))

# su 14 and 8 are odd
node.sub <- subset_draws(as_draws(laa.samples), variable = "lb_mu")
node.sub %>% mcmc_trace()
node.sub %>% summarise_draws(c("mean","median","sd","rhat","ess_bulk","ess_tail")) %>% mutate(mean_n = exp(mean))
...
node.sub <- subset_draws(as_draws(laa.samples), variable = c("l_mean_m_bs", "k_mean_m_bs"))
node.sub %>%
  gather_draws(l_mean_m_bs[r], k_mean_m_bs[r]) %>%
  ggplot() +
  geom_density(aes(.value, color = factor(r))) +
  facet_wrap(~.variable)

node.sub <- subset_draws(as_draws(laa.samples), variable = c("lb_mu"))
node.sub %>%
  gather_draws(lb_mu[su]) %>%
...
node.sub <- subset_draws(as_draws(laa.samples), variable = "par_m")
a<-node.sub %>%
  spread_draws(par_m[yr,supar], sep = ",") %>%
  mutate(par_m = par_m,
         su = if_else(supar < ner, supar, supar - ner),
         par = if_else(supar < ner, 1, 2)) %>%
  filter(par == 1) %>%
  ggplot() +
  geom_density(aes(par_m, color = factor(yr)), show.legend = FALSE) + 
  facet_wrap(~factor(su), scales = "free", nrow = 2) 
b <- node.sub %>%
  spread_draws(par_m[yr,supar], sep = ",") %>%
  mutate(par_m = par_m,
         su = if_else(supar < ner, supar, supar - ner),
  ...
  filter(par == 2) %>%
  ggplot() +
  geom_density(aes(par_m, color = factor(yr))) + 
  facet_wrap(~factor(su), scales = "free", nrow = 2)
