...
for(i in 1:length(vs)){
  print(paste0(vs[i]," ",biph05.model$calculate(vs[i])[1] ))
}

# compile model
biph05.c <- compileNimble(biph05.model)

monits = c("sig.l", "sig.lj", "Ustar","lb.mu","g","k.year","linf","l.mu","l.mu.j") #,"bL","bk"),

# configure and build mcmc
biph05.confmcmc <- configureHMC(biph05.c, 
                                 monitors = monits,
                                 useConjugacy = FALSE
                                 )
node.sub <- stnodes[str_detect(stnodes, "sig.l\\[") | str_detect(stnodes, "l.sigj")]
addHMC(biph05.confmcmc, node.sub, replace = TRUE)

biph05.mcmc <- buildMCMC(biph05.confmcmc, project = biph05.model)

# compile mcmc
biph05.mcmcc <- compileNimble(biph05.mcmc, project = biph05.model, resetFunctions = TRUE)

# MCMC samples
t <- Sys.time()
biph05.samples <- runMCMC(biph05.mcmcc, niter = 10000, nburnin = 7000, nchains = 2, thin=2, samplesAsCodaMCMC = TRUE)
Sys.time() - t

# Save samples
saveRDS(biph05.samples, file = paste0(home,"/data/biph05_samples_",Sys.Date(),".RData"))
