...
Using sdmTMB to biuld trndsw over time for salmon lengthat age data. Three models:
* overall length by age trends based. Spatial and sptemp fileds if possible. Include fixed effect of sex
* length by age trends by spatial unit (including sex and spattemp rf not converging)
*
## Load libraries
...
```{r}
sallaa %>% 
  group_by(year, spat.unit, age.type) %>% 
  summarise(mean_length = mean(length_mm),
            sd_length = sd(length_mm)) %>%
  ggplot(aes(year, mean_length, color = age.type)) +
  geom_line() +
...
#   summarise(mean_length = mean(length_mm),
#             sd_length = sd(length_mm), .by = juv.age) %>%
...
sal.temp <- sallaa %>%
  ...
  mutate(tot.age_f = fct_drop(tot.age_f),
         age_su_f = interaction(tot.age_f, spat.unit, drop = TRUE))

sum(is.na(sal.temp$tot.age_f))
unique(sal.temp$tot.age_f)
unique(sal.temp$site)
unique(sal.temp$age_su_f)
unique(sal.temp$spat.unit)
```

```{r mesh and data}
mesh_s <- make_mesh(sal.temp, c("X", "Y"), cutoff = 8) # 5 < cutoff < 15 may work, 8 lower AIC than 10 
plot(mesh_s)
```
...
sm_sm <- 
  sdmTMB(
    data = sal.temp,
    formula = length_mm ~ age.type + tot.age_f + s(year_sc, by = tot.age_f), # works
    #formula = length_mm ~ age.type + spat.unit + s(year_sc, tot.age_sc, by = spat.unit), # no
    #formula = length_mm ~ age.type + tot.age_f + age_su_f + s(year_sc, by = age_su_f), # no
    #formula = length_mm ~ age.type + tot.age_f + spat.unit + s(year_sc, by = tot.age_f), # no
    #formula = length_mm ~ age.type + site + s(year_sc, tot.age_sc, by = site), 
    mesh = mesh_s,
    spatial = "on",
    spatiotemporal = "iid",
    time = "year",
...
sm_sm2 <-
  sdmTMB(
    ...
    #formula = length_mm ~ age.type + spat.unit + s(year_sc, tot.age_sc, by = spat.unit), # no
    #formula = length_mm ~ age.type + tot.age_f + age_su_f + s(year_sc, by = age_su_f), # no
    #formula = length_mm ~ age.type + age_su_f + s(year_sc, by = age_su_f), # no
    #formula = length_mm ~ age.type + tot.age_f + spat.unit + s(year_sc, by = tot.age_f), # no
    #formula = length_mm ~ age.type + site + s(year_sc, tot.age_sc, by = site), #no
    #formula = length_mm ~ age.type + site + s(year_sc, tot.age_f, by = site), #no
    mesh = mesh_s,
    spatial = "on",
    spatiotemporal = "off",
    time = "year",
...
sanity(sm_sm2) # ln_smooth_sigma for age 0 is large, little data?
exp(sm_sm2$par$ln_smooth_sigma) # ln_smooth_sigma for age 0 is much larger than the other.
AIC(sm_sm2)

# no missing years
my <- min(sal.temp$year):max(sal.temp$year)
missing_years <- my[!my %in% unique(sal.temp$year)]

sm_sm3 <-
  sdmTMB(
    data = sal.temp,
    #formula = length_mm ~ spat.unit + tot.age_f + s(year_sc, by = tot.age_f), # yes
    #formula = length_mm ~ spat.unit + s(year_sc, tot.age_sc, by = spat.unit), # no
    #formula = length_mm ~ site + s(year_sc, tot.age, by = site), # no
    #formula = length_mm ~ (1|spat.unit) + s(year_sc, tot.age_sc, by = spat.unit), # yes
    formula = length_mm ~ spat.unit + s(year_sc, tot.age_sc, by = spat.unit), # yes
    #formula = length_mm ~ (1|age_su_f) + s(year_sc, by = age_su_f), # mo
    #formula = length_mm ~ (1|site) + s(year_sc, by = site), # no
    #formula = length_mm ~ age_su_f + s(year_sc, by = tot.age_sc), # yes
    #formula = length_mm ~ tot.age_f + spat.unit + s(year_sc, by = tot.age_f), # no
    #formula = length_mm ~ site + s(year_sc, tot.age_sc, by = site), #no
    #formula = length_mm ~ site + s(year_sc, tot.age_f, by = site), #no
    mesh = mesh_s,
...
sanity(sm_sm3) # ln_smooth_sigma for age 0 is large, little data?
exp(sm_sm3$par$ln_smooth_sigma) # ln_smooth_sigma for age 0 is much larger than the other.
AIC(sm_sm3)

grid_s <- expand.grid(#"age.type" = unique(sm_sm$data$age.type),
  #"tot.age_f" = unique(sm_sm3$data$tot.age_f),
  #"tot.age_sc" = seq(min(sm_sm3$data$tot.age_sc),max(sm_sm3$data$tot.age_sc),
  #                   length.out = length(unique(sm_sm3$data$tot.age_sc))),
  #"year_sc" = seq(min(sm_sm3$data$year_sc),
  #                max(sm_sm3$data$year_sc),
  #                length.out = length(unique(sm_sm3$data$year_sc))),
  "tot.age_sc" = unique(sm_sm3$data$tot.age_sc),
  "year_sc" = unique(sm_sm3$data$year_sc),
  "spat.unit" = unique(sm_sm3$data$spat.unit)) 
"year" = seq(min(sm_sm$data$year),
             max(sm_sm$data$year),
             length.out = length(unique(sm_sm$data$year)))

grid_asu <- expand.grid("age_su_f" = unique(sm_sm3$data$age_su_f),
                        "year_sc" = seq(min(sm_sm3$data$year_sc),
                                         max(sm_sm3$data$year_sc),
                                         length.out = length(unique(sm_sm3$data$year_sc))),
                        "year" = seq(min(sm_sm3$data$year),
                                 max(sm_sm3$data$year),
                                 length.out = length(unique(sm_sm3$data$year))))
                        
pred_sp <-predict(sm_sm3, re_form = NA)
pred_asu <-predict(sm_sm3, newdata = grid_asu, re_form = NA)
```
...
sm_sm3$data <- sm_sm3$data %>%
  mutate(a.su.y = interaction(tot.age, spat.unit, year, sep = "_"))

pred_sp_2 <- pred_sp %>%
  mutate(year = round(year_sc*sd(sm_sm3$data$year) + mean(sm_sm3$data$year)),
         tot.age = round(tot.age_sc*sd(sm_sm3$data$tot.age) + mean(sm_sm3$data$tot.age)),  
         a.su.y = interaction(tot.age, spat.unit, year, sep = "_")) %>%
  filter(a.su.y %in% sm_sm3$data$a.su.y)

unique(sm_sm3$data$a.su.y)
unique(pred_sp_2$a.su.y)

# FIX THE FILTER BY SPAT UNIT TIME SERIES 
#pred_asu %>% 
pred_sp %>%
  ggplot() + 
  geom_line(aes(year, exp(est), color = factor(tot.age))) +
  facet_wrap(~spat.unit, scales = "free_y") +
  scale_x_continuous(breaks = seq(2000, 2020, by = 10)) +
  labs(y = "length [mm]", title = "age.type : both")

# c.f. e.g. au2 in recent years with info in the data prep html. WHY is some data missing, lacking age? 
sm_sm3$data %>%
  filter(year > 2015,
         spat.unit == "Baltic.sea:AU-2")
...
mesh_sj <- make_mesh(sal.temp.j, c("X", "Y"), cutoff = 10)
...
sanity(sm_sm3_j) # ln_smooth_sigma for age 0 is large, little data?
exp(sm_sm3$par$ln_smooth_sigma) # ln_smooth_sigma for age 0 is much larger than the other.
AIC(sm_sm3_j)
...
sm_sm3_j$data <- sm_sm3_j$data %>%
  mutate(a.su.y = interaction(tot.age, spat.unit, year, sep = "_"))

grid_sj <- expand.grid("tot.age_sc" = unique(sm_sm3_j$data$tot.age_sc),
                       "year_sc" = unique(sm_sm3_j$data$year_sc),
                       "spat.unit" = unique(sm_sm3_j$data$spat.unit)) %>%
  mutate(year = round(year_sc*sd(sm_sm3_j$data$year) + mean(sm_sm3_j$data$year)),
         tot.age = round(tot.age_sc*sd(sm_sm3_j$data$tot.age) + mean(sm_sm3_j$data$tot.age)),  
         a.su.y = interaction(tot.age, spat.unit, year, sep = "_")) %>%
  filter(a.su.y %in% sm_sm3_j$data$a.su.y)

pred_sp_j <-predict(sm_sm3_j, newdata = grid_sj, re_form = NA)
...
pred_sp_j %>%
  ...
  facet_wrap(~spat.unit, scales = "free_y") +
  scale_x_continuous(breaks = seq(2000, 2020, by = 10)) +
  labs(y = "length [mm]", title = "age.type : juven.")
...
Mod_n1_sua <-
  sdmTMB(
  data = sal.temp,
  #formula = length_mm ~ tot.age_sc + year_sc + site, #no
  formula = length_mm ~ tot.age_sc + year_sc + spat.unit, # yes
  #time_varying = ~ 1,
  mesh = mesh_s,
  spatial = "off",
  spatiotemporal = "off",
...
Mod_n1_sub <- 
  sdmTMB(
  data = sal.temp,
  formula = length_mm ~ tot.age_f + year_sc + spat.unit, # yes
  #formula = length_mm ~ tot.age_f + year_sc + site, # no
  #time_varying = ~ 1,
  mesh = mesh_s,
  spatial = "off",
  spatiotemporal = "off",
...
Mod_n1_suc <-  
  sdmTMB( 
  data = sal.temp,
  #formula = length_mm ~ tot.age_f*year_sc + spat.unit, #no
  formula = length_mm ~ tot.age_f*year_sc + site, #no
  #time_varying = ~ 1,
  mesh = mesh_s,
  spatial = "off",
...
Mod_n5c <-
  ...
  data = sal.temp,
  formula = length_mm ~ 0,
  time_varying = ~ 1 + tot.age_sc + spat.unit, # nope
  mesh = mesh_s,
  spatial = "on",
  spatiotemporal = "iid",
  time = "year",
