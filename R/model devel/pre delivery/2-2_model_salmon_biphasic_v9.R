...
#   if(is.na(biph9.model$calculate(deterNodes[i])))
#      {print(paste0(deterNodes[i]," ",biph9.model$calculate(deterNodes[i])))}
# }
# 
# # simulate vars
# for(i in 1:length(dataNodes)){
#   biph9.model$simulate(mvars[i])
#   print(paste0(mvars[i],"",values(biph9.model, mvars[i])))
# }
# for(i in 1:length(deterNodes)){
#   biph9.model$simulate(deterNodes[i])
#   if(sum(is.na(values(biph9.model, deterNodes[i])))>0)
#   {print(deterNodes[i])}
# }
...
#biph9.confmcmc <- configureMCMC(biph9.c, monitors = c("sig.l", "Ustar","lb.mu","smp","g","sig.par","smo.age","mu.par","par", "l.mu", "log_p.year"),
                               # useConjugacy = FALSE)
biph9.confmcmc <- configureMCMC(biph9.c, monitors = c("sig.l", "Ustar","Ugstar","lb.mu","g","log_p.year","smo.age"), enableWAIC = TRUE,#,"bL","bk"),
                                useConjugacy = FALSE)
...
# compile mcmc
t <- Sys.time()
biph9.mcmcc <- compileNimble(biph9.mcmc, project = biph9.model)
Sys.time() - t
...
biph9.samples_bLKJ <- runMCMC(biph9.mcmcc, niter = 5000, nburnin = 1500, nchains = 2, samplesAsCodaMCMC = TRUE, WAIC = TRUE)
...
Sys.time() - t
biph9.samples_bLKJ$WAIC
gelman.diag(biph9.samples_bLKJ$samples)
#gc()
