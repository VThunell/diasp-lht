...
```{r}
laa.samples <- readRDS("/Users/vitl0001/gitProjects/diasp-lht/data/biph1_0_samples_2026-03-07.RData")$samples
biph1_0.samples$WAIC # 529776.1
biph1_0.samples$WAIC # 312954.1 10/3
laa.samples <- biph1_0.samples$samples
...
```{r}
node.sub <- grep("sig_l", colnames(laa.samples[[1]]), value = TRUE)
...
dfj <- laa.samples %>%
  spread_draws(l_mu_j[i], sep = ",") %>%
  median_qi() %>%
  rename(pred.l = l_mu_j,
         ind.id = i) %>%
  ungroup() %>%
  left_join(data.j %>%
...
tmp %>%
  ...
  mutate(linf = exp(linf),
         k = exp(k))
  expand_grid(age = 0:5) %>%
  mutate(length = exp(linf)*(1 - exp(-exp(k)))) %>%
  ggplot() +
  geom_line(aes(x= age, y  = length, color = factor(su))) +
  geom_point(data = data.j, aes(g.year, length_mm_j, color = factor(su))) +
  facet_wrap(~su)
...
data.fec <- salfec %>%
  ...
  drop_na(spat.unit) %>%
  mutate(# count type, 1 is all eggs, 0 is stripped only
         c.type = if_else(is.na(n.eggs.tot), 0, 1), 
         n.eggs = if_else(is.na(n.eggs.tot), n.eggs.str, n.eggs.tot),
         su = as.integer(factor(spat.unit)),
         length.lc = log(length_mm) - log(mean(length_mm)))

data.fec %>%
  drop_na(n.eggs.rem) %>%
  summarise(n(), .by = spat.unit)
  
data.fec %>%
  drop_na(n.eggs.rem) %>%
  mutate(percrem = n.eggs.rem/n.eggs.tot) %>%
  ggplot() +
  geom_boxplot(aes(x = percrem))
  summarise(maxrem = max(percrem), minrem = min(percrem), meanrem = mean(percrem))
...
data.fec %>%
  ...
  mutate(percrem = n.eggs.rem/n.eggs.tot) %>%
  ggplot() +
  geom_boxplot(aes(x = n.eggs.str, y = spat.unit, color = factor(c.type)) +
  geom_boxplot(aes(x = n.eggs.tot, y = spat.unit))
```
...
### Source models / Load samples
```{r}
# readRDS("/Users/vitl0001/gitProjects/diasp-lht/data/fec_samples_2026-01-15.RData")$WAIC
fec.samples <- readRDS("/Users/vitl0001/gitProjects/diasp-lht/data/fec_samples_2026-01-15.RData")$samples
...
node.sub <- grep("^(mu.a|mu.b|sd.a|sd.b)", colnames(fec.samples[[1]]), value = TRUE)
fec.samples[, node.sub[], drop = FALSE] %>%
mcmc_trace()
fec.samples[, node.sub[], drop = FALSE] %>%
gelman.diag()
