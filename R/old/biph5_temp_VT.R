...
data <- sallaa %>%
  mutate(smo.age = if_else(age.type == "both", juv.age, NA),
         ...
         age.index = as.integer(tot.age + 1),
...
# configure..
biph5.confmcmc <- configureMCMC(biph5.c, monitors = c("sig.l", "Ustar", "lb.mu","smp","g","sig.par","smo.age","mu.par","par", "l.mu"), useConjugacy = FALSE, project = biph5.model)

# and build MCMC
biph5.mcmc <- buildMCMC(biph5.confmcmc, project = biph5.model)

# compile mcmc
biph5.mcmcc <- compileNimble(biph5.mcmc, project = biph5.model, resetFunctions = TRUE)

# MCMC Samples
biph5.mcmcc$run(40000, time = TRUE, nburnin=30000,thin = 5)
#biph5.samp <- runMCMC(biph5.mcmcc, niter = 30000, nburnin = 20000, nchains = 1, samplesAsCodaMCMC = TRUE, WAIC = TRUE, thin = 5)
b5.samples <- as.matrix(biph5.mcmcc$mvSamples)
calculateWAIC(b5.samples, biph5.model)
...
vars_subset <- grep("^l.mu\\[", varnames(samp), value = TRUE)
samp.lmu <- samp[, vars_subset, drop = FALSE]
# remove NA nodes (the older-than-catch-age nodes)
cm <- lapply(biph5.samp$samples, as.matrix)
#cm <- biph5.con.samp.lmu$samples
nonNA_nodes <- cm %>% 
...
biph5.con.code <- nimbleCode({
  ...
  for(i in 1:n.ind){
    ...
    l.mu[1,i] <- lb.mu[spat.unit[i]] +  
      (1-step(1-(tot.age.2[i]+0.0001)))*exp(g[spat.unit[i], hatch.year.f[i]]) + step(1-(tot.age.2[i]+0.0001))*exp(g[spat.unit[i], hatch.year.f[i]])*doy.dec[i]
    
    
    #length prediction 1+ and older
    for(a in 2:(age.index[i]+1)){
      l.mu[a,i] <- l.mu[(a-1),i] + 
        (1-step(a-(tot.age.2[i]+0.0001)))*(step(smo.age[i] - (a-0.999))*exp(g[spat.unit[i], (hatch.year.f[i]+a-1)]) + (1-step(smo.age[i] - (a-0.999)))*((exp(par[spat.unit[i],(hatch.year.f[i]+(a-1)),1]) - l.mu[(a-1),i])*(1 - exp(-exp(par[spat.unit[i],(hatch.year.f[i]+(a-1)),2]))))) +
        
        step(a-(tot.age.2[i]+0.0001))*(step(smo.age[i] - (a-0.999))*exp(g[spat.unit[i], (hatch.year.f[i]+a-1)])*doy.dec[i] + (1-step(smo.age[i] - (a-0.999)))*((exp(par[spat.unit[i],(hatch.year.f[i]+(a-1)),1]) - l.mu[(a-1),i])*(1 - exp(-exp(par[spat.unit[i],(hatch.year.f[i]+(a-1)),2])*doy.dec[i]))))
...
# configure..
#biph5.con.confmcmc <- configureMCMC(biph5.con.c, monitors = c("g","par","lb.mu","sig.l","l.mu","smo.age"), project = biph5.con.model, useConjugacy = FALSE, enableWAIC = TRUE)
biph5.con.confmcmc <- configureMCMC(biph5.con.c, monitors = c("sig.l", "Ustar","lb.mu","smp","g","sig.par","smo.age","mu.par","par", "l.mu"), project = biph5.con.model, useConjugacy = FALSE, enableWAIC = TRUE)
...
# MCMC Samples
#biph5.con.samp <- runMCMC(biph5.con.mcmcc, niter = 30000, nburnin = 20000, nchains = 1, samplesAsCodaMCMC = TRUE, WAIC = TRUE, thin = 5)
biph5.con.mcmcc$run(40000, time = TRUE, nburnin=30000,thin = 5)
#biph5.samp <- runMCMC(biph5.mcmcc, niter = 30000, nburnin = 20000, nchains = 1, samplesAsCodaMCMC = TRUE, WAIC = TRUE, thin = 5)
b5.con.samples <- as.matrix(biph5.con.mcmcc$mvSamples)
calculateWAIC(b5.con.samples, biph5.con.model)
...
# remove NA nodes (the older-than-catch-age nodes)
#cm <- lapply(b5.con.samples, as.matrix)
nonNA_nodes <- b5.con.samples %>% 
  map(~ colnames(.x)[colSums(is.na(.x)) == 0]) %>%
  reduce(intersect) # applies function intersect over elements of the lists returned by map(), i.e. removing the colnames from both lists.
cm_cleaned <- lapply(cm, function(x) x[, nonNA_nodes, drop = FALSE])
rm(cm) # big one
