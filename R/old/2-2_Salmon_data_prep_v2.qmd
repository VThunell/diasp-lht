...
pkgs <- c("tidyverse", "tidylog", "devtools","viridis","patchwork", "sdmTMB", "stringi","readxl") 
...
#   dplyr::select("sai_cou_code", "fi_year", "sai_location", "origin", "length_mm", "weight_g", "AU", "sea_age_year", "juvenile_age_year", "sex") %>%
...
#                      ) %>% 
#               drop_na(length_mm) %>%
#               dplyr::select("sai_cou_code", "fi_year", "sai_location", "origin", "length_mm", "sea_age_year", "juvenile_age_year", "sex") %>%
#               mutate(weight_g = NA) %>%
#               left_join(fra.rivers2)) %>%
#   bind_rows(lva.sallaa %>% 
#               mutate(length_mm = if_else(`method_length(0=total,1=fork length)` == 1, exp(0.2892351)*length_mm^0.9623479, length_mm)) %>%
#               dplyr::select("sai_cou_code", "fi_year", "sai_location", "origin", "length_mm", "weight_g", "sea_age_year", "juvenile_age_year","sex", "fisa_y_4326", "fisa_x_4326") %>% 
#               mutate(asses.unit = 5,
#                      region = "Baltic.sea"))
# 
# str(all.sallaa)
...
fra.sallaa.b <- fra.sallaa %>% 
  ...
  drop_na(length_mm) %>%
  dplyr::select(sai_cou_code, fi_year, sai_location, origin, length_mm, sea_age_year,
                juvenile_age_year, sex) %>%
  mutate(weight_g = NA) %>%
  bind_rows(fra.sallaa2 %>% 
              ...
              dplyr::select(sai_cou_code, fi_year, sai_location, origin, length_mm, sea_age_year,juvenile_age_year, sex, sai_lfs_code) %>%
              mutate(weight_g = NA)) %>%
  left_join(fra.rivers2)
...
all.sallaa2 <- all.sallaa %>% 
  ...
  mutate(age.type = case_when(is.na(sea_age_year) & is.na(juvenile_age_year) ~ NA,
                              is.na(sea_age_year) ~ "juve.only", #FEL??
                              sea_age_year > 0 & juvenile_age_year > 0 ~ "both",
                              is.na(juvenile_age_year) ~ "sea.only",
                              .default = NA))

# check n and sizes of unclassified age types. 
all.sallaa2 %>%
  filter(!(is.na(sea_age_year) & is.na(juvenile_age_year))) %>%
  filter(is.na(age.type)) %>%
  distinct(juvenile_age_year, sea_age_year)
all.sallaa2 %>%
  filter(!(is.na(sea_age_year) & is.na(juvenile_age_year))) %>%
  filter(is.na(age.type)) %>%
  ggplot() +
  geom_point(aes(juvenile_age_year, length_mm)) +
  geom_point(aes(sea_age_year, length_mm)) +
  xlab("sea or juvenile age")

# all are too large to be pre smolts so set age.type "both"
all.sallaa3 <- all.sallaa2 %>%
  mutate(age.type = case_when(!(is.na(sea_age_year) & is.na(juvenile_age_year)) & is.na(age.type) ~ "both",
         .default = age.type)) %>%
  # and calculate total age
  rowwise() %>% # rowwise needed for non vectorised function like sum
  mutate(tot_age_year = if_else(is.na(sea_age_year) & is.na(juvenile_age_year), NA,
                           sum(sea_age_year,juvenile_age_year, na.rm = TRUE))) %>%
  ungroup()
