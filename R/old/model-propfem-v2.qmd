...
By instead building this as a linear model with predictors (origin or age seperately below) I use priors with normal distributions on the parameters (alpha and beta)
...
Using the binomial as likelihood and beta as prior (beta-binomial distribution?), the posterior is beta. In this case the likelihood is the success of being female out all individuals (number of trials).
...
## Model 6 - River dependent hierarchial beta binomial with an age predictor is no good because a beta prior doesnt worl on a logit scale which is required to keep the model within 0 and 1. 
...
I need to truncate the beta prior but I the posterior exceeds 1! so a npn beta prior instead and the relationship prop females and age is not linear but more quadratic. Lets try add a polynomial effect.
...
propfem_model_7 <- "model{
  ...
  for(i in 1:nobs){
    ...
    logit(p[i]) <- alpha[ river2[i] ] + b1*age_ad[i] + b2*age_ad[i]^2 
...
```
These posteriors are no good either