...
#biph9.confmcmc <- configureMCMC(biph9.c, monitors = c("sig.l", "Ustar","lb.mu","smp","g","sig.par","smo.age","mu.par","par", "l.mu", "log_p.year"),
                               # useConjugacy = FALSE)
biph9.confmcmc <- configureMCMC(biph9.c, monitors = c("sig.l", "Ustar","Ugstar","lb.mu","g","log_p.year","smo.age"),#,"bL","bk"),
                                useConjugacy = FALSE)
...
biph9.samples <- runMCMC(biph9.mcmcc, niter = 4000, nburnin = 3900, thin=10)#, WAIC=TRUE) 
#Sys.time() - t
#gc()
